...
// FONCTION QUI VA CHERCHER TOUTE LES IMAGES / SECTIONS
function getSlides() {
  if (document.getElementsByClassName) {
    slideImages = document.getElementsByClassName("section");
  } else //IE8
  {
    slideImages = document.querySelectorAll(".section");
  }
...
function createSelectorBtns() {
  ...
  for (var i = 0; i < numSlides; i++) {
    var selectorID = "Page" + (i+1);
    var progBarID = "prog-bar" + (i+1);
    var btn = document.createElement("a");
    btn.setAttribute('href','#'+selectorID);
    btn.classList.add('selector-btn');
    btn.dataset.anchor = '#' + selectorID;
    btn.id = selectorID;
    if (i == numSlides - 1) {
